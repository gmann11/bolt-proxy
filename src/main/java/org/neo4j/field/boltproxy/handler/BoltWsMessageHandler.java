...
public class BoltWsMessageHandler {
    private static Logger log = LoggerFactory.getLogger(BoltWsMessageHandler.class);
    private WsBinaryMessageContext bctx = null;
    private WsConnectContext ctx = null;
    private BoltWsBackend bp = null;
    private Configuration configs = null;
    
    public BoltWsMessageHandler(WsBinaryMessageContext c) {
        bctx = c;   
    }
    public BoltWsMessageHandler(WsConnectContext c, Configuration cfg) {
        ctx = c;
        try {
            configs = cfg;
            log.debug("Creating new backend ws connection");
            bp = new BoltWsBackend(new URI(configs.getString("boltproxy.backend.bolt","ws://localhost:7687")));
            bp.setHandler(this);
